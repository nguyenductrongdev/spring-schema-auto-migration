    ...
    void appliesCreateAndAlterMigrationsIdempotently() {
        ...
        try (CqlSession session = newSession(KEYSPACE)) {
            ...
            MigrationPlan createPlan = comparator.compare(
                    KEYSPACE, initial, inspector.inspect(session, KEYSPACE), true);
            ...
            assertThat(comparator.compare(
                    KEYSPACE, initial, inspector.inspect(session, KEYSPACE), true).isEmpty()).isTrue();

            CassandraSchema evolved = schema(true);
            MigrationPlan alterPlan = comparator.compare(
                    KEYSPACE, evolved, inspector.inspect(session, KEYSPACE), true);
            ...
            assertThat(comparator.compare(
                    KEYSPACE, evolved, inspector.inspect(session, KEYSPACE), true).isEmpty()).isTrue();
        }
    }
