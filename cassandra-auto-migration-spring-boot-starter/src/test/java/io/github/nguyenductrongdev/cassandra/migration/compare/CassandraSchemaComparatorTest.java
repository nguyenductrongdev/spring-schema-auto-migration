package io.github.nguyenductrongdev.cassandra.migration.compare;
...
class CassandraSchemaComparatorTest {
    ...
    @Test
    void reportsUnmanagedObjectsOnlyWhenEnabled() {
        ...
        MigrationPlan reported = comparator.compare("sample", CassandraSchema.empty(), existing, true);
        MigrationPlan ignored = comparator.compare("sample", CassandraSchema.empty(), existing, false);

        assertThat(reported.unsupportedDifferences())
                .extracting(difference -> difference.type())
    ...
    void producesEmptyPlanForMatchingSchema() {
        ...
        assertThat(comparator.compare("sample", schema, schema, true).isEmpty()).isTrue();
    }
    ...
    void preservesCaseSensitiveUdtNamesDuringTypeComparison() {
        ...
        MigrationPlan plan = comparator.compare(
                "sample", schema(List.of(desired), List.of()), schema(List.of(existing), List.of()), false);
    ...
    void preservesInternalKeyspaceCaseWhenGeneratingCql() {
        ...
        MigrationPlan plan = comparator.compare(
                "TenantData", schema(List.of(customer), List.of()), CassandraSchema.empty(), false);

        assertThat(plan.supportedCql().getFirst())
    ...
    void createsTablesWithClusteringOrderAndReportsExistingOrderChanges() {
        ...
        MigrationPlan createPlan = comparator.compare(
                "sample", schema(List.of(desired), List.of()), CassandraSchema.empty(), false);
    ...
    void createsAndAddsStaticColumnsButNeverChangesColumnKind() {
        ...
        assertThat(createPlan.supportedCql().getFirst()).contains("\"category\" text STATIC");
